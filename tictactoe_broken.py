...
def check_winner(game):
    ...
    return 0 # Correctly return 0 if no winner exists (main loop condition)
...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player # Removed the column addition (shifts everything up...)
    return game
...
def switch_player(player):
    if player == 1: # Now uses comparison operator
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: # Missing semicolon
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row, column) # Missing Parameter for function
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game) # uncommented this line
